...
- **调研模式**（产品调研）：只给网址、无代码，agent 探索站点后出 case 草案，**实际走一遍产品核心功能**，逐 case 录屏产出**调研报告**（使用场景、操作方法、交付结果）。**禁止修改目标站任何东西**；真实支付/删除/大量注册/对外发送需用户明示；走不通的路径记为发现/限制，不进 auto-fix。模式判定见 Step 0。
